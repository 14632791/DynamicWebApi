...
* 功能描述：XML文档操作管理器
*
* Author：陈刚 Time：2016-03-10 10:16:26 
*
* 修改标识：
* 修改描述：
*
...
namespace Southernfund.UpdateSystem.Model.Util
{
    ...
    public class XMLHelper
    {
        ...
        public static XmlNode GetXmlNodeByXpath(string xmlFileName, string xpath)
        {
            ...
            {
                LogHelper.ErrorLog("XMLHelper->GetXmlNodeByXpath", ex);
                return null;
        ...
        public static XmlNodeList GetXmlNodeListByXpath(string xmlFileName, string xpath)
        {
            ...
            {
                LogHelper.ErrorLog("XMLHelper->GetXmlNodeListByXpath", ex);
                return null;
                //throw ex; //这里可以定义你自己的异常处理
            }
        }
        ...
        public static bool CreateXmlDocument(string xmlFileName, string rootNodeName, string version= "1.0", string encoding= "utf-8", string standalone=null)
        {
        ...
        /// <param name="innerText">节点文本值</param>
        /// <returns>成功返回true,失败返回false</returns>
        public static bool CreateOrUpdateXmlNodeByXPath(XmlDocument xmlDoc, string xpath, string directory, string name, string innerText)
        {
            bool isSuccess = false;
            bool isExistsNode = false;//标识节点是否存在
            try
            {
                XmlNode xmlNode = xmlDoc.SelectSingleNode("//FileList");
                if (xmlNode != null)
                {
                    //遍历xpath节点下的所有子节点
                    foreach (XmlNode node in xmlNode.ChildNodes)
                    {
                        if (node["Directory"].ToString().ToLower() == directory.ToLower() && node["Name"].ToString().ToLower() == name.ToLower())
                        {
                            //存在此节点则更新
                            node.InnerXml = innerText;
                            isExistsNode = true;
                    ...
                    if (!isExistsNode)
                    {
                        ...
                        xeFile.SetAttribute("HashMD5", innerText);
                        xeFile.SetAttribute("Directory", directory);
                        xeFile.SetAttribute("Name", name);
                        xmlDoc.AppendChild(xeFile);
                    }
                }
                isSuccess = true;
            }
